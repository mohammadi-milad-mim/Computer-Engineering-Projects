import sys
from TXT_PROCESS import binary_to_hex, hb2i, hex_to_binary, hexstring_to_byte, i2hb


Sbox = (
    ,
)

InvSbox = (
    ,
)

Rcon = [0x00000000, 0x01000000, 0x02000000,
		0x04000000, 0x08000000, 0x10000000, 
		0x20000000, 0x40000000, 0x80000000, 
		0x1b000000, 0x36000000]

xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)

def byte_xor(ba1, ba2):
    return bytes([_a ^ _b for _a, _b in zip(ba1, ba2)])

def bitwise_xor_bytes(a, b):
    result_int = int.from_bytes(a, byteorder="big") ^ int.from_bytes(b, byteorder="big")
    return result_int.to_bytes(max(len(a), len(b)), byteorder="big")

def shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]

def inv_shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]
        


def mix_columns(s):
    for i in range(4):
        t = hb2i(s[i][0]) ^ hb2i(s[i][1]) ^ hb2i(s[i][2]) ^ hb2i(s[i][3])
        u = hb2i(s[i][0])
        t0 = hb2i(s[i][0]) ^ t ^ xtime(hb2i(s[i][0]) ^ hb2i(s[i][1]))
        t1 = hb2i(s[i][1]) ^ t ^ xtime(hb2i(s[i][1]) ^ hb2i(s[i][2]))
        t2 = hb2i(s[i][2])^ t ^ xtime(hb2i(s[i][2]) ^ hb2i(s[i][3]))
        t3 = hb2i(s[i][3])^ t ^ xtime(hb2i(s[i][3]) ^ u)
        s[i][0], s[i][1], s[i][2], s[i][3] = i2hb(t0), i2hb(t1), i2hb(t2), i2hb(t3)
        
def inv_mix_columns(s):
        for i in range(4):
            u = xtime(xtime(hb2i(s[i][0]) ^ hb2i(s[i][2])))
            v = xtime(xtime(hb2i(s[i][1]) ^ hb2i(s[i][3])))
            t0 = hb2i(s[i][0]) ^ u
            t1 = hb2i(s[i][1]) ^ v
            t2 = hb2i(s[i][2]) ^ u
            t3 = hb2i(s[i][3]) ^ v
            s[i][0],s[i][1],s[i][2],s[i][3]= i2hb(t0), i2hb(t1), i2hb(t2), i2hb(t3)

        mix_columns(s)
        
def sub_bytes(s):
    for i in range(4):
        for j in range(4):
            #print(s[i][j])
            xo = Sbox[int(s[i][j], 16)]
            #print(xo)
            hx = hex(xo)
            hxb = hx[2:]
            res = bytes(hxb, 'utf-8')
            #print(s[i][j])
            #print(res)
            s[i][j]=res
            #print(s[i][j])


def inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            xo = InvSbox[hb2i(s[i][j])]
            res = i2hb(xo)
            s[i][j] = res
            
def encrypt2(var, key, byteorder=sys.byteorder):
        key, var = key[:len(var)], var[:len(key)]
        int_var = int.from_bytes(var, byteorder)
        int_key = int.from_bytes(key, byteorder)
        #print(int_var)
        #print(int_key)
        int_enc = int_var ^ int_key
        return int_enc.to_bytes(len(var), byteorder)
    
def add_round_key(s, k):
        for i in range(4):
            for j in range(4):
                #print(type(s[i][j]))
                #print(type(k[i][j]))
                #s[i][j] ^= k[i][j]
                #print(s[i][j],type(s[i][j]))
                #print(k[i][j],type(k[i][j]))
                #s[i][j] = bitwise_xor_bytes(s[i][j], k[i][j])
                #print(int(s[i][j], 16))
                #print(int(k[i][j], 16))
                xo = int(s[i][j], 16) ^ int(k[i][j], 16)
                #print(xo)
                hx = hex(xo)
                hxb = hx[2:]
                res = bytes(hxb, 'utf-8')
                #print(s[i][j])
                #print(res)
                s[i][j]=res
                #print(res)
                #x=encrypt2(s[i][j],k[i][j])
                #print(x)                
#takes two hex values and calculates hex1 xor hex2
def hexor(hex1, hex2):
    #convert to binary
	bin1 = hex_to_binary(hex1)
	bin2 = hex_to_binary(hex2)
	
	#calculate
	xord = int(bin1, 2) ^ int(bin2, 2)
	return binary_to_hex(xord)

def toRCON (x):
    return Rcon[int(x/4)]


def RotWord(word):
	return word[1:] + word[:1]
		
		
#selects correct value from sbox based on the current word
def SubWord(word):
	sWord = ()
	
	#loop throug the current word
	for i in range(4):
		
		#check first char, if its a letter(a-f) get corresponding decimal
		#otherwise just take the value and add 1
		if word[i][0].isdigit() == False:
			row = ord(word[i][0]) - 86
		else:
			row = int(word[i][0])+1

		#repeat above for the seoncd char
		if word[i][1].isdigit() == False:
			col = ord(word[i][1]) - 86
		else:
			col = int(word[i][1])+1
		
		#get the index base on row and col (16x16 grid)
		sBoxIndex = (row*16) - (17-col)
		
		#get the value from sbox without prefix
		piece = hex(Sbox[sBoxIndex])[2:]
		
		#check length to ensure leading 0s are not forgotton
		if len(piece) != 2:
			piece = '0' + piece
		
		#form tuple
		sWord = (*sWord, piece)
		
	#return string
	return ''.join(sWord)

